...
import com.example.e_commerce.config.DatabaseConfiguration;
...
public class UserDao {
    ...
    public Function<Users, Boolean> updateUserBalance = (users -> {
        try {
            connect.compile();
        } catch (SQLException | ClassNotFoundException e) {
            logger.warning("SQL exception"+e.getMessage());
        ...
        try {
            ...
            preparedStatement.setDouble(1, users.getBalance().doubleValue());
            return preparedStatement.execute();
        } catch (SQLException e) {
            throw new RuntimeException(e);
        }
